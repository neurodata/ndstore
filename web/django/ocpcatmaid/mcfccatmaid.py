...
class MCFCCatmaid:
  ...
  def cacheMiss ( self, resolution, xtile, ytile, zslice ):
    ...
    return emcarest.mcfcPNG ( self.proj, self.db, self.token, "xy", self.channels, imageargs )


  def getTile ( self, webargs ):
    """Either fetch the file from memcache or get a mcfc image"""

    # parse the web args
    self.token, tileszstr, self.channels, plane, resstr, xtilestr, ytilestr, zslicestr, rest = webargs.split('/',8)

    [ self.db, self.proj, projdb ] = emcarest.loadDBProj ( self.token )

...
TGVERSION = "1.5.1"
TGDOWNLOAD = "http://www.turbogears.org/%s/downloads/%s/" % (
    TGVERSION[:3], TGVERSION)
...
"""
import sys
# this is the default version of setuptools, not TG
...
# TGBEGIN
win32 = """
The recommended Python version for TurboGears 1.5 is Python 2.6.6.
...
other = """
The recommended Python version for TurboGears 1.5 is Python 2.6.6.
This version of Python is often available in your operating system's
native package format (via apt-get or yum, for instance).
You can also easily build Python from source on Unix-like systems.
Here is the source download link for Python:
...
def check_python():
    if sys.version_info < (2, 4):
        print "Python 2.4 or higher is required to run TurboGears 1.5."
    ...
    class easy_install(eimodule.easy_install):
        ...
        def finalize_options(self):
            if self.future:
                ...
                self.args.append("TurboGears == %s" % TGVERSION)
            if self.upgrade is None:
                self.upgrade = True
            if self.script_dir is None:
                if os.name == "posix":
                    if self.prefix:
                        #honor the prefix specified on the command line
                        self.script_dir = os.path.join(self.prefix, "bin")
                else:
                    ...
                    if not re.search(r'Python\d+\\Script', path):
                        ...
                        post_install_msgs.append((
                          "You may need to update the PATH in your "
                          'environment to allow you to run the "tg-admin" '
                          "command or any other Python scripts that you "
                          "install.\n\nTo do that on Windows:\n\n"
                          "Go to System Properties -> Advanced -> Environment"
                          " Variables and add or edit the PATH variable to "
                          "include "
                          "';C:\Python%(py_ver)s;C:\Python%(py_ver)s\Scripts'"
                          "assuming you installed Python to the default "
                          "location.") % {'py_ver': py_ver})

            if self.find_links is None:
                self.find_links = []
            elif isinstance(self.find_links, basestring):
